...
$startInfo.Arguments = "scripts\official_dashboard_publisher.py --env-file `"$sourceEnvFile`" --local-url `"$localDashboardUrl`" --remote-url `"$remotePublishUrl`" --interval $interval --repeat-interval 2.0 --remote-timeout 2.0 --urgent-signal --urgent-retry-interval 0.2 --non-entry-urgent-interval 0.2 --full-dashboard --log-file official_dashboard_publisher.log"
