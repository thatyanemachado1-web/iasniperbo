...
export function calendarResultEventsFromLegacyEngineEvent(value: unknown) {
  ...
  if (
    ...
    !eventKey ||
    !occurredAt ||
    (outcome !== "green" && outcome !== "red" && outcome !== "tie")
  ) {
    return [];
  }
...
function buildMonthSummary(
  ...
  const counts = days.reduce<Record<CalendarAuditClassification, number>>(
    (acc, day) => {
      const value = readString(day.classification);
      const key: CalendarAuditClassification =
        value === "muito_pagante" ||
        value === "operavel" ||
        value === "perigoso" ||
        value === "sem_amostra"
