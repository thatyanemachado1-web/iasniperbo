...
export function detectAiPatternsConfirmedCard(
  ...
  const snapshot = readRecord(
    (dashboard as DashboardData & { patternMinerSnapshot?: unknown }).patternMinerSnapshot ||
      dashboard.patternMiner,
  );
  ...
  if (exactResults.length) {
    return exactResults
      .map((value, index) => {
        ...
        if (!side || !Number.isFinite(valueNumber) || slot < 0 || slot >= 156) return null;
        return {
        ...
        } satisfies LateralBacBoResult;
      })
      .filter((result): result is LateralBacBoResult => Boolean(result))
