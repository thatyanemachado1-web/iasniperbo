...
function AdminPage() {
  ...
  const [ownerEmail, setOwnerEmail] = useState(
    savedAdminSession?.email || (canAccessAdminPanel(userSession.email) ? userSession.email : ""),
  );
  const canUseAdmin = Boolean(ownerEmail || savedAdminSession?.email || userSession.email);
  const [session, setSession] = useState<AdminSession | null>(() => savedAdminSession);
...
function recipientToEditForm(recipient: SignalRecipient): RecipientEditForm {
  return {
    ...
    access_status: (["approved", "paused", "pending"] as const).includes(
      recipient.access_status as "approved" | "paused" | "pending",
    )
      ? (recipient.access_status as "approved" | "paused" | "pending")
      : recipient.enabled
        ? "approved"
